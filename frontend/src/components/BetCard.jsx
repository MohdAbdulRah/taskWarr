...
const BetCard = ({ bet }) => {
  ...
  return (
    <div
    className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-md border border-gray-200 p-6 transition-transform hover:scale-[1.02] duration-300 w-full max-w-md mx-auto
               h-[500px] sm:h-[480px] md:h-[460px] lg:h-[450px] flex flex-col justify-between"
    onClick={() => navigate(`/bet/${bet._id}`)}
  >
    <div>
      <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">{bet.title}</h2>
    ...
    <div className="flex items-center justify-between mt-4">
      ...
      {bet.status === "pending" && (
        <button
          ...
          className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold px-4 py-2 rounded-full text-xs md:text-sm transition duration-300 shadow-sm hover:shadow-md"
        >
          Accept Task
        </button>
      )}
